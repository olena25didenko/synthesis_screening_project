#!/usr/bin/env python3
"""
Phase 4: Codon Deoptimization
Use the RAREST codon for each amino acid (opposite of codon optimization)

Usage:
    python phase_4_codon_deoptimization.py \
        --input-fasta "path/to/original_sequences.fasta" \
        --blast-db "path/to/blast/reference_db" \
        --output "phase_4_results.json"
"""
...
# Build reverse: AA -> sorted list of codons (sorted = rarest first alphabetically)
AA_TO_CODONS = defaultdict(list)
for codon, aa in CODON_TABLE.items():
    AA_TO_CODONS[aa].append(codon)

# Sort codons for each AA (first = rarest, last = most common, approximately)
for aa in AA_TO_CODONS:
    AA_TO_CODONS[aa].sort()

def read_fasta(filepath):
    """Read FASTA file and return dict {seq_id: dna_sequence}"""
    sequences = {}
    current_id = None
    current_seq = []
    ...
    with open(filepath, 'r') as f:
        for line in f:
            ...
            if line.startswith('>'):
                ...
                current_id = line[1:].split()[0]  # Use first token as ID
                current_seq = []
            else:
...
def deoptimize_codons(dna_seq):
    """Use RAREST codon for each amino acid"""
    modified = []
    ...
    for i in range(0, len(dna_seq) - 2, 3):
        ...
        if len(codon) == 3 and codon in CODON_TABLE:
            ...
            # Use rarest codon (first in sorted list)
            rarest = AA_TO_CODONS[aa][0]
            modified.append(rarest)
        else:
...
def calculate_nucleotide_identity(original, modified):
    """Calculate nucleotide identity percentage"""
...
def run_blastn(query_seq, blast_db_path):
    """Run BLASTN and return detection result"""
    try:
...
def main():
    ...
    DEFAULT_INPUT = r"C:\Users\diden\synthesis_screening_project\data\reference_sequences"
    DEFAULT_BLAST_DB = r"C:\Users\diden\synthesis_screening_project\data\blast_db"
    DEFAULT_OUTPUT = r"C:\Users\diden\synthesis_screening_project\results\phase_4_results.json"
    DEFAULT_MODIFIED_SEQ = r"C:\Users\diden\synthesis_screening_project\results\phase_4_modified_sequences.fasta"
    
    parser = argparse.ArgumentParser(description='Phase 4: Codon Deoptimization')
    ...
    results = {
        'technique': 'phase_4_codon_deoptimization',
    ...
    for seq_id, original_seq in sorted(sequences.items()):
        # Apply deoptimization
        modified_seq = deoptimize_codons(original_seq)
    ...
    if nucleotide_identities:
        results['nucleotide_identity_stats'] = {
            ...
            'std': (sum((x - sum(nucleotide_identities)/len(nucleotide_identities))**2 
                        for x in nucleotide_identities) / len(nucleotide_identities)) ** 0.5
        }
    ...
    print(f"Std Dev: {results['nucleotide_identity_stats']['std']:.1%}")
