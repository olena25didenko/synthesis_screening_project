#!/usr/bin/env python3
"""
Phase 5: Dinucleotide Shuffling
Rearrange dinucleotide patterns within codons while preserving amino acids

Usage:
    python phase_5_dinucleotide_shuffling.py \
        --input-fasta "path/to/original_sequences.fasta" \
        --blast-db "path/to/blast/reference_db" \
        --output "phase_5_results.json"
"""
...
def shuffle_dinucleotides(dna_seq):
    """Shuffle dinucleotide patterns while preserving amino acids"""
    ...
    for i in range(0, len(dna_seq) - 2, 3):
        ...
        if len(codon) == 3 and codon in CODON_TABLE:
            ...
            # Pick random alternative codon for this AA
            if AA_TO_CODONS[aa]:
                alt_codon = random.choice(AA_TO_CODONS[aa])
                
                # Shuffle the dinucleotides in the alternative codon
                bases = list(alt_codon)
                random.shuffle(bases)
                shuffled_codon = ''.join(bases)
                
                # Verify it codes for same AA (if not, use original alternative)
                if shuffled_codon not in CODON_TABLE or CODON_TABLE[shuffled_codon] != aa:
                    shuffled_codon = alt_codon
                
                modified.append(shuffled_codon)
            else:
...
def main():
    ...
    DEFAULT_OUTPUT = r"C:\Users\diden\synthesis_screening_project\results\phase_5_results.json"
    DEFAULT_MODIFIED_SEQ = r"C:\Users\diden\synthesis_screening_project\results\phase_5_modified_sequences.fasta"
    
    parser = argparse.ArgumentParser(description='Phase 5: Dinucleotide Shuffling')
    ...
    results = {
        'technique': 'phase_5_dinucleotide_shuffling',
    ...
    for seq_id, original_seq in sorted(sequences.items()):
        # Apply shuffling
        modified_seq = shuffle_dinucleotides(original_seq)
